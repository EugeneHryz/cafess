...
public class ClientDaoImpl extends ClientDao {
    ...
    private static final String SQL_CREATE_CLIENT = "INSERT INTO clients(name, surname, role_id, status_id, email, password, " +
            "balance, profile_image) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

    private static final String SQL_FIND_ALL_CLIENTS = "SELECT " +
            "clients.id, name, surname, email, password, role, status, balance, profile_image FROM clients " +
    ...
    private static final String SQL_UPDATE_CLIENT = "UPDATE clients SET name = ?, surname = ?, " +
            "email = ?, password = ?, role_id = ?, status_id = ?, balance = ?, profile_image = ? " +
            "WHERE clients.id = ?";
    ...
    public boolean create(Client entity) throws DaoException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(SQL_CREATE_CLIENT, Statement.RETURN_GENERATED_KEYS)) {
            ...
            statement.setBlob(8, entity.getProfileImage());
